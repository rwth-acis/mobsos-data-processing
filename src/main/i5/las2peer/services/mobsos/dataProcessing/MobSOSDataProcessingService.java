...
/**
 * This service is responsible for processing incoming monitoring data. It tests
 * the data for correctness and stores them in a relational database. The
 * provision is done by the Monitoring Data Provision Service.
 */
@ManualDeployment
public class MobSOSDataProcessingService extends Service {
  ...
  /**
   * Will be called by the receiving {@link i5.las2peer.security.MonitoringAgent}
   * of this service, if it receives a message from a monitored node.
   *
   * @param messages an array of
   *                 {@link i5.las2peer.logging.monitoring.MonitoringMessage}s
  ...
  /**
   * Checks the messages content and calls
   * {@link #persistMessage(MonitoringMessage, String)} with the corresponding
   * values.
   *
   * @param messages an array of
   *                 {@link i5.las2peer.logging.monitoring.MonitoringMessage}s
  ...
  private boolean processMessages(MonitoringMessage[] messages) {
    ...
    for (MonitoringMessage message : messages) {
      ...
      } else if ((message.getEvent() == MonitoringEvent.NODE_STATUS_CHANGE && message.getRemarks().equals("RUNNING"))) {
      ...
      else if (message.getEvent() == MonitoringEvent.NODE_STATUS_CHANGE && message.getRemarks().equals("CLOSING")) {
      ...
      else if (message.getEvent() == MonitoringEvent.AGENT_REGISTERED && !message.getRemarks().equals("ServiceAgent")
          ...
          try {
            ...
            if (jo instanceof JSONObject) {
              ...
              if (function != null && hasBot() && triggerFunctions.contains(function.toLowerCase())) {
                BotMessage m = new BotMessage(message.getTimestamp(), message.getEvent(), message.getSourceNode(),
                    message.getSourceAgentId(), message.getDestinationNode(), message.getDestinationAgentId(),
                    message.getRemarks());
                botMessages.add(m);
              }
            }
   ...
   * {@link DatabaseInsertStatement} helper class. It then calls the database for
   * persistence.
   *
   * @param message           a
   *                          {@link i5.las2peer.logging.monitoring.MonitoringMessage}
   * @param table             the table to insert to. This parameter does
   *                          determine what action will be performed on the
   *                          database (insert an agent, a message, a node..).
   * @param exceptionHandling Set to true to enable exception recovery after a
   *                          failed database interaction.
   * @return true, if message persistence did work
   */
      ...
      if (exceptionHandling) {
        try {
          ...
          if ("REGISTERED_AT".equals(table)) {
            ...
            PreparedStatement nodeStatement = DatabaseQuery.returnNodeQueryStatement(con, message.getSourceNode(),
            ...
            if (!result.next()) {
              ...
              PreparedStatement insertStatement = DatabaseInsertStatement.returnInsertStatement(con, message,
                  database.getJdbcInfo(), "NODE", hashRemarks);
  ...
  /**
   * Returns the id of this monitoring agent (that will be responsible for message
   * receiving). Creates one if not existent.
   *
   * @param greetings will be printed in the console and is only used to control
   *                  registering
   * @return the id
   */
